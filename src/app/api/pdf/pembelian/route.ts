...
export async function GET(req: NextRequest) {
  ...
  const { data: purchases } = await supabase
    ...
    .select(`id, kode_pembelian, tanggal, total, status_pembayaran,
  ...
  let totalFaktur = 0, totalPembelian = 0, totalSisaHutang = 0, totalQty = 0

  const rows = (purchases || []).map((p, i) => {
    totalFaktur++
    totalPembelian += p.total
    let qty = 0
    p.purchase_items?.forEach((it: any) => { qty += it.qty })
    let paid = 0
    p.supplier_payments?.forEach((sp: any) => { paid += sp.nominal })
    const sisa = p.total - paid
    totalQty += qty
  ...
  const tableRows = rows.map(r => {
    const badgeClass = r.status_pembayaran === 'LUNAS' ? 'badge-lunas' : r.status_pembayaran === 'PARSIAL' ? 'badge-parsial' : 'badge-hutang'
    return `<tr>
      ...
      <td class="right green">${formatRp(r.sudah_bayar)}</td>
      <td class="right red">${formatRp(r.sisa_hutang)}</td>
      <td class="center"><span class="badge ${badgeClass}">${r.status_pembayaran}</span></td>
    </tr>`
  }).join('')
...
<div class="page">
  ...
  <div class="summary">
    <div class="summary-box"><div class="summary-label">Total Faktur</div><div class="summary-value">${totalFaktur}</div></div>
    <div class="summary-box"><div class="summary-label">Total Produk Masuk (Qty)</div><div class="summary-value">${totalQty}</div></div>
    <div class="summary-box orange"><div class="summary-label">Total Nilai Pembelian</div><div class="summary-value">${formatRp(totalPembelian)}</div></div>
    <div class="summary-box red"><div class="summary-label">Total Sisa Hutang</div><div class="summary-value">${formatRp(totalSisaHutang)}</div></div>
    <div class="summary-box green"><div class="summary-label">Total Sudah Dibayar</div><div class="summary-value">${formatRp(totalPembelian - totalSisaHutang)}</div></div>
  </div>
  <div class="section-title">Rincian Transaksi Pembelian</div>
  <table>
    <thead><tr>
      <th class="center" style="width:30px">No</th>
      <th>Tanggal</th><th>Kode Faktur</th><th>Supplier</th>
      <th class="center">Qty</th><th class="right">Total Nilai</th>
      <th class="right">Sudah Dibayar</th><th class="right">Sisa Hutang</th>
      <th class="center">Status</th>
    </tr></thead>
    <tbody>${tableRows || '<tr><td colspan="9" class="center" style="padding:20px;color:#999">Tidak ada data pembelian pada periode ini</td></tr>'}</tbody>
    <tfoot><tr>
      <td colspan="4" style="text-align:right">TOTAL</td>
      <td class="center">${totalQty}</td>
      <td class="right">${formatRp(totalPembelian)}</td>
      <td class="right">${formatRp(totalPembelian - totalSisaHutang)}</td>
      <td class="right">${formatRp(totalSisaHutang)}</td>
      <td></td>
    </tr></tfoot>
  </table>
  <div class="footer">
