...
export async function GET(req: NextRequest) {
  ...
  const tab = searchParams.get('tab') || 'utama'
  ...
  if (tab === 'utama') {
    ...
    pendapatanBersih = pendapatanKotor - totalDiskon
  ...
  } else if (tab === 'aki-bekas') {
    titleSuffix = '(Aki Bekas)'
    const { data: abSales } = await supabase.from('aki_bekas_sales').select('total, hpp_total')
      .eq('status', 'POSTED').gte('tanggal', startDate).lte('tanggal', endDate)

    abSales?.forEach(s => {
      pendapatanKotor += (s.total || 0)
      totalHPP += (s.hpp_total || 0)
    })
    
    pendapatanBersih = pendapatanKotor
    labaKotor = pendapatanBersih - totalHPP
    bebanOperasional = 0 // Aki Bekas has no operational expenses
